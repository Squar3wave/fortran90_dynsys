...
function slicer_k(arr_in, i_in)

real(dp), intent(in)     :: arr_in(0:,0:)
integer, intent(in)      :: i_in
real(dp), dimension(:,:) :: slicer_k(0:3)

slicer_k(0) = arr_in(i_in,0)
slicer_k(1) = arr_in(i_in,1)
slicer_k(2) = arr_in(i_in,2)
slicer_k(3) = arr_in(i_in,3)

end function

function deslicer_k(arr_in, i_in)

real(dp), intent(in)     :: arr_in(0:)
integer, intent(in)      :: i_in
real(dp), dimension(:,:) :: deslicer_k(0:2, 0:3)

deslicer_k(i_in,0) = arr_in(0)
deslicer_k(i_in,1) = arr_in(1)
deslicer_k(i_in,2) = arr_in(2)
deslicer_k(i_in,3) = arr_in(3)
